...
# Coverageplot wrapper to simplify group.by user choices in server module
# cluster names to be controlled by user input (cluster_browser to be able to subset plot)
# extend_window to set to 0 in case of region != gene (aka coordinates)
# downsample.rate speeds it up a bit at the small cost of resolution (default 0.1). CoverageBrowser does the same
CoveragePlot_ratlas <- function(Seurat_object, split_type = "All", region, extend_window = 0, show_bulk, idents, assay = assay) {
  if (split_type == "All") {
    CoveragePlot(
      ...
      annotation = TRUE,
      peaks = TRUE,
      extend.upstream = extend_window,
      extend.downstream = extend_window,
      downsample.rate = 0.02,
      max.downsample = 2000, # also set to same as CoverageBrowser
      show.bulk = show_bulk,
      group.by = NULL,
      idents = idents
    )} else if (split_type == "CellType.Stim") {
      
      # by default no selections which pass on NULL (fine if not splitting),
      # but here have to ensure it's not NULL if user would like to see all clusters
      
      if (is.null(idents)) {
        idents <- sort(as.character(unique(Seurat_object@meta.data$CellType)))
        idents_by_split_type <- sort(c(paste0(idents, "_Saline"), paste0(idents, "_Cocaine")))
      } else {
        idents_by_split_type <- sort(c(paste0(idents, "_Saline"), paste0(idents, "_Cocaine"))) 
      }
      ...
      CoveragePlot(
        ...
        show.bulk = show_bulk,
        group.by = split_type,
        idents = idents_by_split_type
      )} else {
        
        if (is.null(idents)) {
          idents <- sort(as.character(unique(Seurat_object@meta.data$CellType)))
          idents_by_split_type <- sort(c(paste0(idents, "_Female"), paste0(idents, "_Male")))
        } else {
          idents_by_split_type <- sort(c(paste0(idents, "_Female"), paste0(idents, "_Male"))) 
        }
